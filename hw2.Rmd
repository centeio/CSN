...
### INTRODUCTION
We are asked here to analyse the degree distribution of global syntatic dependency networks (one network for each language). In other words vertices of these networks are words (word types) and two vertices ( two words ) are linked if they have been linked at least once in a dependency treebank.

In details, the degree distributions of each language can follow an unknown distribution that we want try to catch/approximate. What we want to do then is called model selection: we start with a bunch of proposal distributions choosen a priori (possibly parametric distribution in order to use the tuning of the parameter to find the best fit for our data) evaluate their best parameter(s) according to the data with Maximum Likelihood Estimation and then compare the proposal models with the Akaike Informatio Criterion and see which one fit best our data.

The model suggested in the task are
...
Since we are not allowing for unconnected node (e.g unconnected words) some changes on the original implementation of Poisson distribution and Geometric were needed
...
The code below read each undirected network corresponding to one of each language and evaluate the MLE for the parameter(s) of each candidate distribution. After that it evaluates the AIC criterion of the model and compare them finding the best AIC. The model selection is carry out with and without including the Altmann function. In this way we can check the goodness of this last model to approximate degree distribution of words. Visual fitting is generated for each language and each model used. 
...
Looking at the AIC table showed (the one without the Altmann function) it is easy to see how the zeta distribution function is able to approximate better than the other candidates the unknown degree distribution. Geometric distribution is the one that after the zeta better approximate the underlying distribuion. Right truncated zeta and Poisson (particularly the last one) are far from catching a good approximation. This can be seen also visually.. Poisson distribution is always vary different from shape of the histogram of the data.
...
When plotting we faced the problem of giving a good data visualization. All the languages present long tail because exists few words that show a really high degree. To avoid this issue, when plotting, we choose to show up to the 0.95 percentile of the distribution discarding the last 5% of the distribution located in high values of degree.
...
Notice however that the values of the Right truncated distribution can be affected by the not properly implementation of the MLE optimization. Since we were getting errors optimizing with respect to the Kmax value following the suggestion of the Prof Marta Arias we split the optimization proces in two different parts: first we optimize first the $\gamma$ parameter keeping Kmax fixed to the higher degree value found in the degree distribution; then keeping $\gamma = \gamma_{MLE}$ we optimize the second parameter Kmax. We are aware is not a proper optimization process but at least allow us to get some values to use in comparison part.
